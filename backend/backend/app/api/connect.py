...
async def get_profiling_progress(fingerprint: str):
    job_queue = get_durable_job_queue()
    return job_queue.get_job_progress(fingerprint)
...
async def connect_by_config(body: ConnectionConfigRequest):
    ...
    try:
        ...
        fingerprint = schema_service._get_db_fingerprint()
        job_queue = get_durable_job_queue()
        job_queue.submit_onboarding_job(database_url=url, fingerprint=fingerprint)
...
async def reconnect_profile(connection_id: str):
...
async def connect_by_url(body: ConnectURLRequest):
    ...
    try:
        ...
        fingerprint = schema_service._get_db_fingerprint()
        job_queue = get_durable_job_queue()
        job_queue.submit_onboarding_job(database_url=db_url, fingerprint=fingerprint)
...
async def connect_by_preset(body: ConnectPresetRequest):
...
async def connect_by_upload(file: UploadFile = File(...)):
