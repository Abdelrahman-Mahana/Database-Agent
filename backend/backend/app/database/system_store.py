...
class SystemStore:
    ...
    def get_catalog_connection(self, fingerprint: str) -> Optional[Any]:
        ...
        from app.schema_catalog.models import DatabaseConnectionRecord
        try:
            with self.engine.connect() as conn:
                row = conn.execute(
                    text("""
    ...
    def get_schema_objects(self, fingerprint: str, table_names: Optional[list[str]] = None) -> list[Any]:
        ...
        from app.schema_catalog.models import SchemaObjectRecord
        try:
            with self.engine.connect() as conn:
                if table_names:
    ...
    def get_columns_for_objects(self, fingerprint: str, object_ids: Optional[list[str]] = None) -> list[Any]:
        ...
        from app.schema_catalog.models import ColumnRecord
        try:
            with self.engine.connect() as conn:
                if object_ids:
    ...
    def get_relationships(self, fingerprint: str) -> list[Any]:
        ...
        from app.schema_catalog.models import RelationshipRecord
        try:
            with self.engine.connect() as conn:
                rows = conn.execute(
                    text("""
    ...
    def get_indexes(self, fingerprint: str) -> list[Any]:
        ...
        from app.schema_catalog.models import IndexStatsRecord
        try:
            with self.engine.connect() as conn:
                rows = conn.execute(
                    text("""
    ...
    def get_aliases(self, fingerprint: str) -> list[Any]:
        ...
        from app.schema_catalog.models import AliasTermRecord
        try:
            with self.engine.connect() as conn:
                rows = conn.execute(
                    text("""
    ...
    def get_latest_catalog_version(self, fingerprint: str) -> Optional[Any]:
        ...
        from app.schema_catalog.models import CatalogVersionRecord
        try:
            with self.engine.connect() as conn:
                row = conn.execute(
                    text("""
    ...
    def load_normalized_catalog(self, fingerprint: str) -> Optional[Any]:
        ...
        from app.schema_catalog.models import SchemaCatalog, DatabaseConnectionRecord
        try:
    ...
    def load_table_subset(self, fingerprint: str, table_names: list[str]) -> dict[str, Any]:
        ...
        from app.schema_catalog.models import TableProfile, ColumnProfile
        if not table_names:
            return {}
        try:
