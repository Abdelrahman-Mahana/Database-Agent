...
def test_schema_grounding_engine_adaptive_capacity():
    ...
    # Must be bounded to adaptive capacity (<= 15 tables max), not all 30!
...
def test_centrality_fallback_is_not_relevance_evidence():
    ...
    grounded = SchemaGroundingEngine().build_grounded_schema(
        schema=schema,
        question="unrelated terminology with no catalog evidence",
    )

    score, detail = grounding_confidence(grounded)

    assert grounded.fallback_used is True
    assert detail["fallback_used"] is True
    assert detail["fallback_penalty"] == 0.30
    assert score < 1.0


def test_complex_enterprise_10_table_chain_preserved():
    """Verify that an enterprise schema requiring a 10-table join chain preserves all 10 tables and join paths."""
    # 10-table chain: t0 -> t1 -> t2 -> ... -> t9
    schema = {}
    for i in range(10):
        tname = f"ent_t{i}"
        fks = []
        if i > 0:
            fks.append({
                "constrained_columns": ["prev_id"],
                "referred_table": f"ent_t{i-1}",
                "referred_columns": ["id"],
            })
        schema[tname] = {
            "columns": [
                {"name": "id", "type": "int", "primary_key": True},
                {"name": "prev_id", "type": "int"},
    ...
    engine = SchemaGroundingEngine()
    qu = QueryUnderstanding(
        raw_question="Calculate relationship between ent_t0 and ent_t9",
        entities=["ent_t0", "ent_t9"],
    )

    grounded = engine.build_grounded_schema(
        schema=schema,
        query_understanding=qu,
        question="Calculate relationship between ent_t0 and ent_t9",
    )

    # All 10 tables in the chain must be preserved
    assert len(grounded.selected_tables) == 10
    for i in range(10):
        assert f"ent_t{i}" in grounded.selected_tables

    # All 9 join paths connecting the chain must be present in schema_text
    assert "Join Paths:" in grounded.schema_text
    for i in range(1, 10):
        join_a = f"ent_t{i}.prev_id = ent_t{i-1}.id"
        join_b = f"ent_t{i-1}.id = ent_t{i}.prev_id"
        assert join_a in grounded.schema_text or join_b in grounded.schema_text
