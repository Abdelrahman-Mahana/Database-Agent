...
class QuerySpecBuilder:
    ...
    def _build_deterministic_data_spec(
        ...
        if schema and db_ctx is not None:
            ...
            if any(term in q_lower for term in ("doctor", "doctors", "\u0627\u0644\u0623\u0637\u0628\u0627\u0621", "\u0627\u0637\u0628\u0627\u0621", "\u062f\u0643\u062a\u0648\u0631", "\u0637\u0628\u064a\u0628")):
                add_table_anchor("doctor_model")
            ...
            if explicit_tables:
                ...
                for table_name in db_ctx.match_seed_tables_fast(
                    ...
                    if any(
                        ...
                        candidate_table_set.add(table_name)
            elif db_ctx.keyword_to_tables and not has_business_anchor:
    ...
    def _merge_llm_and_deterministic_specs(
        ...
        if deterministic_spec.expected_output == OutputFormat.SCALAR and deterministic_spec.aggregations == ["COUNT"]:
            merged.entities = deterministic_spec.entities or llm_spec.entities
    ...
    def _quick_route(
        ...
        has_data_cue = has_data_cue or has_arabic_data_cue
