...
class ClickHouseConnector(DatabaseConnector):
    ...
    async def discover(self) -> DatabaseMetadata:
        engine = create_engine("clickhouse://dummy")
        inspector = InspectorFactory.create_inspector("generic_sqlalchemy")
        return inspector.inspect(engine, "clickhouse_db")

    def get_engine(self):
        return create_engine("clickhouse://dummy")
    ...
    def build_sample_query(
        ...
        if strategy == "random":
            query += f" SAMPLE {sample_size} LIMIT {sample_size}"
        elif strategy == "limit":
