...
def grounding_confidence(grounded: Any, query_spec: Any = None) -> tuple[float, dict[str, Any]]:
    ...
    selected_tables = list(getattr(grounded, "selected_tables", []) or [])
    selected_columns = getattr(grounded, "selected_columns", {}) or {}
    if score == 0.0 and selected_tables and not fallback_used and not ambiguous:
        # Some fast/mock paths provide a grounded subset without the optional
        # evidence map. A selected compact schema is still a useful grounding
        # signal; reserve perfect scores for explicit evidence.
        score = 0.75 if selected_columns else 0.65
