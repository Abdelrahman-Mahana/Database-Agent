...
from app.services.analysis.models import AnalysisPlan
...
class Planner:
    ...
    def __init__(self, primary_llm, fast_llm):
        ...
        self.analysis_planner = AnalysisPlanner(fast_llm)
    ...
    async def decompose_question(
        ...
        """Decompose a complex question into sub-questions using AnalysisPlan first."""
        # 1. Check if an AnalysisPlan can be derived from QuerySpec
        if query_spec is not None and getattr(query_spec, "analysis_required", False):
            try:
                analysis_plan = await self.analysis_planner.plan_async(
                    query_spec=query_spec,
                    schema_text=schema_text,
                    conversation_history=conversation_history,
                )
                sub_qs = analysis_plan.get_sub_questions()
                if sub_qs and len(sub_qs) >= 2:
                    logger.info("Using %d data retrieval sub-questions from AnalysisPlan", len(sub_qs))
                    return sub_qs
            except Exception as e:
                logger.warning("AnalysisPlanner decomposition failed, falling back to LLM chain: %s", e)

        # 2. Fallback to LLM decompose chain
        try:
    ...
    async def execute_plan(
        ...
        if step_contexts:
            analytical_findings = []
            if analysis_plan is not None and execution_results:
                try:
                    analysis_exec_result = self.analysis_executor.execute(analysis_plan, execution_results)
                    if analysis_exec_result.all_findings:
                        analytical_findings = analysis_exec_result.all_findings
                except Exception as e:
                    logger.warning("AnalysisExecutor execution failed: %s", e)

            final_context_str = "\n".join(
                f"Step {i+1}: Q: {s['q']} -> SQL: {s['sql']} -> Results: {s['summary']}"
                for i, s in enumerate(step_contexts)
            )
            if analytical_findings:
                final_context_str += "\n\nVerified Analytical Computations & Findings:\n" + "\n".join(f"- {f}" for f in analytical_findings)
