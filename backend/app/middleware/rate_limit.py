...
def consume_rate_limit_ram(client_key: str, max_requests: int, window_seconds: float = 60.0) -> bool:
    """Fast, thread-safe in-RAM token bucket rate limiter without SQLite overhead."""
    now = time.monotonic()
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        allowed = consume_rate_limit_ram(client_key, self.requests_per_minute, window_seconds=60.0)
