...
class ResultVerifier:
    ...
    def _requested_metric_columns(self, rows: list[dict[str, Any]], query_spec: Optional[Any], question: str) -> list[str]:
        ...
        for column in columns:
            ...
            if not self._is_numeric_value(value) or self._is_identifier_column(column):
                continue
            ...
            if (
                ...
                or any(m in column.lower() for m in ("revenue", "amount", "total", "sum", "price", "sales", "cost", "salary", "balance"))
            ):
    ...
    def _is_ranking_request(self, query_spec: Optional[Any], sql: str, question: str) -> bool:
        ...
        return (
            shape == "ranking"
            or bool(re.search(r"\b(top|bottom|highest|lowest|best|worst|most|least)\b|أعلى|الأكثر|أقل|الأدنى|افضل|أفضل", question, re.I))
            or bool(re.search(r"\bORDER\s+BY\s+(?:SUM|COUNT|AVG|MAX|MIN)\b", sql, re.I))
        )
    ...
    def _ranking_facts(self, rows: list[dict[str, Any]], query_spec: Optional[Any], question: str, sql: str) -> list[DeterministicFact]:
        ...
        if not self._is_ranking_request(query_spec, sql, question):
            return []

        # Only use explicitly requested or aggregated metric columns
        ...
        # Prioritize columns that contain real string labels
        def _score_entity(col: str) -> int:
            cl = col.lower()
            if cl in ("name", "full_name", "display_name", "customer_name", "patient_name", "user_name"):
                return 100
            if "name" in cl:
                return 80
            if cl in ("month", "date", "year", "period", "day", "time_period", "quarter"):
                return 75
            if cl in ("title", "label", "description", "email", "code", "ref", "reference", "category"):
                return 60
            if not self._is_identifier_column(col) and cl != "rank":
                return 20
            return 0
        ...
        for col in rows[0]:
            if col != metric:
                ...
                if sample_vals and any(isinstance(v, str) and v.strip() for v in sample_vals):
                    candidate_cols.append(col)

        candidate_cols.sort(key=_score_entity, reverse=True)
        entity_column = candidate_cols[0] if candidate_cols else None
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if not entity or str(entity).strip().lower() in ("none", "null", ""):
                continue
            value = row.get(metric)
            rank_label = row.get("rank") or row.get("rank_label") or row.get("highest_month") or row.get("lowest_month")
            rank_prefix = f" ({rank_label})" if rank_label and isinstance(rank_label, str) else ""
            statement = f"Rank {index}{rank_prefix}: {entity} — {metric.replace('_', ' ')} is {value}."
            facts.append(DeterministicFact(
                claim_id=f"fact_rank_{index}_{uuid.uuid4().hex[:8]}",
                fact_type="ranked_entity",
                statement=statement,
                source_column=metric,
                source_value={"rank": index, "rank_label": rank_label, "entity": str(entity), "metric": value},
                operation="rank",
    ...
    def verify(
        ...
        at = getattr(query_spec, "analysis_type", None)
        at_val = at.value if hasattr(at, "value") else str(at).lower()
        is_row_ranking = ("ORDER BY" in sql_upper and "LIMIT" in sql_upper)
        if query_spec and at_val not in ("exploratory_analysis", "data_quality", "lookup", "ranking", "retrieval") and not is_row_ranking:
            aggregations = getattr(query_spec, "aggregations", []) or []
            metrics = getattr(query_spec, "metrics", []) or []
            has_aggregates = any(fn in sql_upper for fn in ("COUNT(", "SUM(", "AVG(", "MIN(", "MAX("))
            if (metrics or aggregations) and not has_aggregates and "GROUP BY" not in sql_upper:
    ...
    def verify_and_constrain_prose(
        ...
        sentences = [s.strip() for s in re.split(r"(?<!\d)[.!?]\s+|\n+", report_text.strip()) if s.strip()]
        evaluations: list[ClaimEvaluation] = []
        valid_sentences: list[str] = []

        CAUSAL_AND_SPECULATIVE_PATTERNS = re.compile(
            r"(?:السبب\s+(?:هو|يرجع|يعود)|يرجع\s+ذلك\s+إلى|بسبب|نتيجة\s+(?:لـ|إلى)|أدى\s+ذلك\s+إلى|تراجع\s+بسبب|انخفض\s+بسبب|ضعف\s+(?:التسويق|المبيعات|الأداء|الإقبال)|caused\s+by|due\s+to|the\s+reason\s+is|because(?:\s+of)?|driven\s+by|attributable\s+to|resulting\s+from|marketing\s+(?:campaign|budget|spend)|competitor\s+(?:action|pricing))",
            re.IGNORECASE,
        )

        # Build evidence vocabulary pool across all 4 pillars:
        # 1. SQL results (columns, values)
        # 2. Deterministic calculations (facts)
        # 3. Statistical / Analytical results (findings, metrics, evidence)
        # 4. Schema evidence
        evidence_vocab: set[str] = set()
        for r in rows:
            for k, v in r.items():
                evidence_vocab.add(str(k).lower().strip())
                if isinstance(v, str):
                    evidence_vocab.add(v.lower().strip())
                    for token in v.lower().split():
                        evidence_vocab.add(token.strip(".,;:()[]\"'"))

        if facts:
            for f in facts:
                stmt = f.statement if hasattr(f, "statement") else str(f)
                for token in stmt.lower().split():
                    evidence_vocab.add(token.strip(".,;:()[]\"'"))

        if analytics_result:
            if hasattr(analytics_result, "findings") and analytics_result.findings:
                for find in analytics_result.findings:
                    for token in str(find).lower().split():
                        evidence_vocab.add(token.strip(".,;:()[]\"'"))
            if hasattr(analytics_result, "evidence") and analytics_result.evidence:
                for ev in analytics_result.evidence:
                    for token in str(ev).lower().split():
                        evidence_vocab.add(token.strip(".,;:()[]\"'"))
            if hasattr(analytics_result, "metrics") and analytics_result.metrics:
                for mk, mv in analytics_result.metrics.items():
                    evidence_vocab.add(str(mk).lower().strip())
                    evidence_vocab.add(str(mv).lower().strip())

        number_pattern = re.compile(r"(?:\$)?\b\d+(?:[,\.\s\u202f\u00a0]\d+)*(?:%)?")

        def _parse_localized_number(raw_match: str) -> Optional[float]:
            raw_str = raw_match.strip().strip("$% ")
        ...
        # Collect numbers mentioned in the user's question or SQL (e.g. thresholds like "5", "10", "2024")
        context_numbers = set()
        for text_source in (question, sql):
            if text_source:
                for m in number_pattern.findall(text_source):
                    v = _parse_localized_number(m)
                    if v is not None:
                        context_numbers.add(v)
        ...
        for sent in sentences:
            ...
            # Check for ungrounded causal/analytical assertions (e.g. "السبب هو marketing")
            is_causal_assertion = bool(CAUSAL_AND_SPECULATIVE_PATTERNS.search(sent_clean))
            has_causal_evidence = False
            if is_causal_assertion:
                # Check if specific causal tokens are grounded in the evidence vocabulary
                words = [w.strip(".,;:()[]\"'").lower() for w in sent_clean.split() if len(w) > 3]
                matched_tokens = [w for w in words if w in evidence_vocab]
                has_causal_evidence = len(matched_tokens) >= 2

            nums = number_pattern.findall(sent_clean)
            metric_candidates: list[Tuple[str, float]] = []
            for match in nums:
                val = _parse_localized_number(match)
                if val is None:
                    continue
                # Ignore numbers from question or SQL filter thresholds
                if val in context_numbers:
                    continue
                # Ignore temporal calendar years (1900-2099), list bullet indices, and date components
                if val.is_integer():
                    int_v = int(val)
                    if 1900 <= int_v <= 2099:
                        continue
                    # Ignore list enumeration indices at the start of lines (e.g. "1.", "2.", "1-", "1)")
                    if re.match(rf"^\s*{re.escape(match)}[\.\-\)]\s+", sent_clean):
                        continue
                    # Ignore row count if matches total rows
                    if int_v == len(rows):
                        continue
                    # Ignore month/day numbers inside date patterns like 2024-08-14, 2024-07, 2024/07/15
                    if re.search(rf"\b\d{{4}}[-/]\d{{1,2}}[-/]{re.escape(match)}\b|\b{re.escape(match)}[-/]\d{{1,2}}[-/]\d{{4}}\b|\b\d{{4}}[-/]{re.escape(match)}\b|\b{re.escape(match)}[-/]\d{{4}}\b", sent_clean):
                        continue
                    if (1 <= int_v <= 31) and any(kw in sent_clean.lower() for kw in ("شهر", "الشهور", "month", "date", "تاريخ", "#", "المركز", "ترتيب", "أعلى", "أقل", "أدنى", "rank", "top", "bottom")):
                        continue
                metric_candidates.append((match, val))

            matched_evidence: list[dict[str, Any]] = []
            unmatched_numbers: list[str] = []

            for match, val in metric_candidates:
            ...
            # Numeric-free narrative assertions without causal claims:
            # Recognize structural headers, list labels, and conversational framing
            is_structural_markdown = (
                not metric_candidates
                and not is_causal_assertion
                and (
                    sent_clean.endswith(":")
                    or sent_clean.startswith(("#", "-", "*", ">"))
                    or len(sent_clean.split()) <= 20
                    or bool(re.search(r"\b(here (are|is)|results?|details?|records?|data|database|calculated|verify|sql|raw result|لو|إذا|تحب|حابب|قولي|فترات|استعراض|أخرى|جاهز|المسار|النتائج|ملاحظات|المسجلة|الفواتير|الإيرادات|الشهرية|ملخص|النقاط|تغيرات|ملحوظة|مبني|إجمالي|أعلى|أدنى|أقل|نلاحظ|تباين|قفزة|واضحة|ارتفاع|تراجع|انخفاض)\b", sent_clean, re.I))
                )
            )
            framing_text = not metric_candidates and not is_causal_assertion and is_structural_markdown
            is_claim_valid = bool(matched_evidence) and not unmatched_numbers
            if is_causal_assertion and not has_causal_evidence:
                evaluations.append(ClaimEvaluation(
                    statement=sent_clean,
                    status="UNSUPPORTED_CLAIM",
                    is_verified=False,
                    confidence=0.0,
                    evidence_source="unsupported_claim: causal speculation not backed by schema or database evidence",
                ))
                valid_sentences.append(f"{sent_clean} *(unsupported claim)*")
            elif is_claim_valid:
        ...
        # If all claims are verified, preserve original formatted report text cleanly
        if all(e.is_verified for e in evaluations):
            return report_text, evaluations, overall_confidence

        # Otherwise, tag only unverified claims in the original prose while preserving formatting
        constrained_prose = report_text
        for e in evaluations:
            if not e.is_verified and e.statement:
                tag = " *(unsupported claim)*" if e.status == "UNSUPPORTED_CLAIM" else " *(unverified claim)*"
                if tag not in constrained_prose and e.statement in constrained_prose:
                    constrained_prose = constrained_prose.replace(e.statement, f"{e.statement}{tag}")

        return constrained_prose, evaluations, overall_confidence

    @staticmethod
    def _normalise_words(value: Any) -> str:
        return re.sub(r"[^\w]+|_", " ", str(value).lower(), flags=re.UNICODE).strip()
    ...
    def _build_claim_evidence(self, rows: list[dict[str, Any]], sql: str = "") -> list[dict[str, Any]]:
        ...
        for row_index, row in enumerate(rows):
            ...
            for column, value in row.items():
                if not self._is_numeric_value(value) or isinstance(value, bool):
                    continue
                ...
                evidence.append({
                    "value": numeric_val, "entity": entity_values,
                    "metric": column,
        ...
        if row_count > 1:
            for column in rows[0]:
                ...
                if not values or not all(self._is_numeric_value(v) and not isinstance(v, bool) for v in values):
                    continue
    ...
    def _find_claim_evidence(
        ...
        for keyword, candidate_operation in (
            ("average", "average"), ("avg", "average"), ("متوسط", "average"), ("معدل", "average"),
            ("total", "sum"), ("sum", "sum"), ("إجمالي", "sum"), ("مجموع", "sum"),
            ("minimum", "min"), ("lowest", "min"), ("أدنى", "min"), ("أقل", "min"), ("اصغر", "min"), ("أصغر", "min"),
            ("maximum", "max"), ("highest", "max"), ("أعلى", "max"), ("اكبر", "max"), ("أكبر", "max"), ("ذروة", "max"),
        ):
            if keyword in normalized_sentence.split() or keyword in normalized_sentence:
        ...
        for candidate in evidence_index:
            ...
            def matches_entity(ent: str) -> bool:
                norm_ent = self._normalise_words(ent)
                if norm_ent in normalized_sentence:
                    return True
                # Match temporal year/month entities like 2024-07 with 2024 / يوليو / July
                if "-" in ent and len(ent.split("-")) >= 2:
                    parts = ent.split("-")
                    year, month_num = parts[0], parts[1]
                    month_map = {
                        "01": ("يناير", "jan"), "02": ("فبراير", "feb"), "03": ("مارس", "mar"),
                        "04": ("أبريل", "apr"), "05": ("مايو", "may"), "06": ("يونيو", "jun"),
                        "07": ("يوليو", "jul"), "08": ("أغسطس", "aug"), "09": ("سبتمبر", "sep"),
                        "10": ("أكتوبر", "oct"), "11": ("نوفمبر", "nov"), "12": ("ديسمبر", "dec"),
                    }
                    if year in normalized_sentence:
                        if month_num in month_map:
                            ar_m, en_m = month_map[month_num]
                            if ar_m in normalized_sentence or en_m in normalized_sentence:
                                return True
                        return True
                return False

            entity_matched = bool(entities and any(matches_entity(e) for e in entities))
            ...
            if candidate["operation"] == "count":
                ...
                metric_in_sentence = (
                    ...
                    or (
                        ...
                        and bool(re.search(r"\b(has|have|there are|exist|exists|number of|total|across|عدد|إجمالي)\b", normalized_sentence))
                    )
                )
            else:
                metric_in_sentence = self._metric_is_named(metric, normalized_sentence)
                # If entity is matched in a list/bullet row without naming a conflicting metric
                if not metric_in_sentence and entity_matched and candidate["operation"] == "value":
                    # Check if sentence named an unrelated metric
                    unrelated_named = False
                    for other_cand in evidence_index:
                        other_m = self._normalise_words(other_cand["metric"])
                        if other_m != metric and self._metric_is_named(other_m, normalized_sentence):
                            unrelated_named = True
                            break
                    if not unrelated_named:
                        metric_in_sentence = True

            if not metric_in_sentence:
                continue
            op_match = (
                candidate["operation"] == operation
                or candidate["operation"] in ("value", "rank", "highest", "lowest", "max", "min")
                or operation == "value"
                or candidate["operation"] == "count"
            )
            if not op_match:
                continue

            if candidate["operation"] == "value" and not entity_matched:
                # If metric is named and exact value matches, allow single-row / peak-min query matches
                if not metric_in_sentence:
                    continue
            return {
    ...
    def _metric_is_named(metric: str, sentence: str) -> bool:
        if metric and metric in sentence:
            return True
        # Conservative semantic aliases in English and Arabic.
        aliases = {
            "revenue": {"revenue", "sales", "generated", "bringing", "إيراد", "إيرادات", "مبيعات", "دخل", "فواتير"},
            "amount": {"amount", "value", "total", "مبلغ", "قيمة", "إجمالي", "فواتير", "إيراد", "إيرادات"},
            "total_amount": {"amount", "value", "total", "مبلغ", "قيمة", "إجمالي", "إيرادات", "فواتير"},
            "amount_total": {"amount", "value", "total", "مبلغ", "قيمة", "إجمالي", "إيرادات", "فواتير"},
            "total": {"total", "sum", "إجمالي", "مجموع", "كلي", "إيراد", "إيرادات"},
            "units sold": {"units sold", "units", "quantity", "كمية", "عدد"},
            "units_sold": {"units sold", "units", "quantity", "كمية", "عدد"},
            "count": {"count", "total", "number", "عدد", "إجمالي"},
            "price": {"price", "سعر", "تكلفة", "مبلغ"},
            "customer": {"customer", "customers", "client", "clients", "عميل", "عملاء"},
            "customer_count": {"customer", "customers", "client", "clients", "عميل", "عملاء"},
            "product": {"product", "products", "item", "items", "منتج", "منتجات"},
            "product_count": {"product", "products", "item", "items", "منتج", "منتجات"},
            "company": {"company", "companies", "شركة", "شركات"},
            "company_count": {"company", "companies", "شركة", "شركات"},
            "month": {"month", "date", "شهر", "تاريخ", "فترة"},
        }
        cand_aliases = set(aliases.get(metric, set()))
        for m_key, m_set in aliases.items():
            if m_key in metric or metric in m_key:
                cand_aliases |= m_set
        return any(alias in sentence for alias in cand_aliases)
